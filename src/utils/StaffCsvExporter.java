...
public class StaffCsvExporter {
    ...
    public static void exportUserToCsv(User user) {
    ...
    public static void exportUsers(Collection<User> users) {
        // Convert collection to map for efficient lookup
        ...
        try {
            ...
            for (User user : users) {
                if (!processedIds.contains(user.getHospitalID())) {
                    lines.add(formatUserToCsv(user));
                }
            }
